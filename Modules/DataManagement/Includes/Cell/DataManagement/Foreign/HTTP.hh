...
private:
    CELL_INLINE Connection(Network::Socket* socket) : socket(socket) { }
...
public:
    ...
    CELL_FUNCTION static Wrapped<Request*, Result> FromRaw(const IBlock& block);

    CELL_FUNCTION ~Request();

private:
    CELL_INLINE Request(uint8_t* data) : data(data) { }
