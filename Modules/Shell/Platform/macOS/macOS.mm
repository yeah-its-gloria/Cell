...
Wrapped<macOS*, Result> macOS::New(const String& title, const Extent extent) {
    ...
    instance->window.mouseXRef = &instance->mouseX;
    instance->window.mouseYRef = &instance->mouseY;

    instance->window.isLocked = false;
    return instance;
}
...
Result macOS::CaptureState(const bool captured) {
    if (captured == this->window.isLocked) {
        return Result::Success;
    }
