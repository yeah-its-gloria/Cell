...
-(void) mouseMoved: (NSEvent*) event {
    CELL_ASSERT(event.type == NSEventTypeMouseMoved && self.mouseXRef != nullptr && self.mouseYRef != nullptr);

    *self.mouseXRef += event.deltaX;
    *self.mouseYRef += event.deltaY;

    if (self.isLocked) {
        // TODO: return with CGWarpMouseCursorPosition();
    }
}
@end
