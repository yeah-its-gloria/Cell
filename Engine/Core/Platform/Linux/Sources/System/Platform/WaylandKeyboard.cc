...
void Linux::WaylandKeyboardKeymap(void* data, struct wl_keyboard* keyboard, const uint32_t format, const int32_t fd, const uint32_t size) {
    ...
    CELL_ASSERT(map_shm != MAP_FAILED);

    struct xkb_keymap* keymap = xkb_keymap_new_from_string(_linux->keyboardContext, (char*)map_shm, XKB_KEYMAP_FORMAT_TEXT_V1, XKB_KEYMAP_COMPILE_NO_FLAGS);
...
void Linux::WaylandKeyboardKey(void* data, struct wl_keyboard* keyboard, const uint32_t serial, const uint32_t time, const uint32_t key, const uint32_t state) {
    ...
    case 113: // Q
    {
        if (state & WL_KEYBOARD_KEY_STATE_PRESSED) { _linux->keys |= Shell::KeyValue::Q; } else { _linux->keys ^= Shell::KeyValue::Q; }
    ...
    case 119: // W
    {
        if (state & WL_KEYBOARD_KEY_STATE_PRESSED) { _linux->keys |= Shell::KeyValue::W; } else { _linux->keys ^= Shell::KeyValue::W; }
    ...
    case 101: // E
    {
        if (state & WL_KEYBOARD_KEY_STATE_PRESSED) { _linux->keys |= Shell::KeyValue::E; } else { _linux->keys ^= Shell::KeyValue::E; }
    ...
    case 97: // A
    {
        if (state & WL_KEYBOARD_KEY_STATE_PRESSED) { _linux->keys |= Shell::KeyValue::A; } else { _linux->keys ^= Shell::KeyValue::A; }
    ...
    case 115: // S
    {
        if (state & WL_KEYBOARD_KEY_STATE_PRESSED) { _linux->keys |= Shell::KeyValue::S; } else { _linux->keys ^= Shell::KeyValue::S; }
    ...
    case 100: // D
    {
        if (state & WL_KEYBOARD_KEY_STATE_PRESSED) { _linux->keys |= Shell::KeyValue::D; } else { _linux->keys ^= Shell::KeyValue::D; }
...
void Linux::WaylandKeyboardModifiers(void* data, struct wl_keyboard* keyboard, const uint32_t serial, const uint32_t modsDepressed, const uint32_t modsLatched, const uint32_t modsLocked,
